...
<p align="center"> A fabulous shell with its proper terminal !
    <br> 
</p>
...
| `source [-c \| --create \| file_path]`                          | `source [-c \| --create]` that setup the `~/.myshrc`, `source file_path` source the given file path and if no argument is given, automaticly source the `~/.myshrc` |
| `history [number_to_diplay]`                                    | Display all or the given number of the last command used (stoked in `~/.mysh_history`) |
