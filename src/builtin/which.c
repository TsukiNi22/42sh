...
static int display_path(main_data_t *data, char *cmd, char *path)
{
    ...
    for (int i = 0; data->env_path[i]; i++) {
        ...
        if (is_accesible_dir(data->env_path[i]) && is_valid(cmd_p))
            return KO * ((data->pty && pty_display_path(data, cmd_p))
    ...
    char *var[2], bool builtin)
{
    ...
    if (var[0] && !var[1] && !builtin
        ...
        return err_prog(UNDEF_ERR, KO, ERR_INFO);
    if (var[1] && !builtin && ((data->pty && pty_wich(data, cmd, var, true) ==
        KO) || (!data->pty && my_printf("%s='%s'\n", cmd, var[1]) == KO)))
        return err_prog(UNDEF_ERR, KO, ERR_INFO);
